...
// Modules accessibles selon role
const MODULES = [
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#050C1A 0%,#091525 60%,#050C1A 100%)", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'JetBrains Mono','Courier New',monospace", padding:20 }}>
      ...
      <div style={{ width:"100%", maxWidth:420, animation:"fadeUp .5s ease forwards" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:36 }}>
          ...
          <div style={{ fontSize:11, color:"#3E5470", letterSpacing:"0.2em", textTransform:"uppercase", marginTop:4 }}>
            Gestion des Operations Internationales
          </div>
        </div>
        ...
        <div style={{ background:"rgba(8,15,28,.9)", border:"1px solid rgba(255,255,255,.08)", borderRadius:16, padding:32, backdropFilter:"blur(12px)", boxShadow:"0 30px 60px rgba(0,0,0,.5)" }}>
          ...
          <div style={{ marginBottom:16 }}>
            ...
            <input value={login} onChange={e => setLogin(e.target.value)} onKeyDown={handleKey}
              ...
              style={{ width:"100%", background:"rgba(10,18,32,.8)", border:"1.5px solid "+(error?"rgba(239,68,68,.5)":"#1D3250"), borderRadius:9, padding:"10px 14px", fontSize:13, color:"#C8D8EA", fontFamily:"monospace", outline:"none", transition:"border-color .2s" }}
              onFocus={e => e.target.style.borderColor="#2A6090"}
              onBlur={e  => e.target.style.borderColor=error?"rgba(239,68,68,.5)":"#1D3250"} />
          </div>
          ...
          <div style={{ marginBottom:24 }}>
            ...
            <div style={{ position:"relative" }}>
              <input value={password} onChange={e => setPassword(e.target.value)} onKeyDown={handleKey}
                ...
                style={{ width:"100%", background:"rgba(10,18,32,.8)", border:"1.5px solid "+(error?"rgba(239,68,68,.5)":"#1D3250"), borderRadius:9, padding:"10px 40px 10px 14px", fontSize:13, color:"#C8D8EA", fontFamily:"monospace", outline:"none", transition:"border-color .2s" }}
                onFocus={e => e.target.style.borderColor="#2A6090"}
                onBlur={e  => e.target.style.borderColor=error?"rgba(239,68,68,.5)":"#1D3250"} />
              <button onClick={() => setShowPwd(v=>!v)} style={{ position:"absolute", right:12, top:"50%", transform:"translateY(-50%)", background:"none", border:"none", color:"#3E5470", cursor:"pointer", fontSize:14, padding:2 }}>
                {showPwd ? "🙈" : "👁"}
              </button>
            </div>
          </div>
          ...
          <div style={{ marginTop:20, paddingTop:16, borderTop:"1px solid rgba(255,255,255,.06)" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6 }}>
              {USERS.slice(0,6).map(u => (
                <div key={u.id} onClick={() => { setLogin(u.login); setPassword(u.password); setError(""); }}
                  style={{ padding:"6px 10px", borderRadius:7, background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.06)", cursor:"pointer", transition:"all .15s" }}
            ...
            <div style={{ fontSize:9, color:"#1e3a5f", textAlign:"center", marginTop:8 }}>
              Mot de passe demo : Test@1234 (sauf admin : Admin@2026)
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ display:"flex", height:"100vh", fontFamily:"'JetBrains Mono','Courier New',monospace", background:"#050C1A", overflow:"hidden", color:"#C8D8EA" }}>
      ...
      <div style={{ width:navOpen?240:64, flexShrink:0, background:"rgba(6,10,20,.98)", borderRight:"1px solid rgba(255,255,255,.06)", display:"flex", flexDirection:"column", transition:"width .25s ease", overflow:"hidden" }}>
        ...
        <nav style={{ flex:1, padding:"12px 8px", display:"flex", flexDirection:"column", gap:3, overflowY:"auto" }}>
          {accessibles.map(m => {
            ...
            return (
              <button key={m.id} onClick={() => setActive(m.id)} title={!navOpen?m.label:""}
                ...
                {navOpen && <div style={{ textAlign:"left", animation:"fadeIn .2s ease" }}><div style={{ fontSize:12, fontWeight:700, color:active?m.color:"#7A8BA0" }}>{m.label}</div></div>}
              </button>
            );
          })}
          ...
          {user.role==="ADMIN" && (
            <button onClick={() => setShowAdmin(true)} title={!navOpen?"Console Admin":""}
              style={{ display:"flex", alignItems:"center", gap:10, padding:navOpen?"10px 12px":"10px 0", justifyContent:navOpen?"flex-start":"center", borderRadius:9, border:"none", cursor:"pointer", width:"100%", background:"rgba(245,158,11,.06)", borderLeft:"3px solid rgba(245,158,11,.3)", marginTop:8, transition:"all .15s" }}>
        ...
        <div style={{ padding:"10px 8px", borderTop:"1px solid rgba(255,255,255,.05)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"9px 10px", borderRadius:9, background:"rgba(6,182,212,.06)", border:"1px solid rgba(6,182,212,.12)" }}>
            ...
            {navOpen && (
              <div style={{ flex:1, minWidth:0, animation:"fadeIn .2s ease" }}>
          ...
          {navOpen && (
            <button onClick={() => setUser(null)} style={{ width:"100%", marginTop:6, padding:"6px", borderRadius:8, fontSize:10, fontWeight:700, cursor:"pointer", background:"rgba(239,68,68,.06)", border:"1px solid rgba(239,68,68,.15)", color:"#ef4444" }}>
              Deconnexion
            </button>
          )}
        </div>
        ...
        {/* Toggle nav */}
        <button onClick={()=>setNavOpen(v=>!v)} style={{ padding:"10px", background:"rgba(255,255,255,.02)", border:"none", borderTop:"1px solid rgba(255,255,255,.05)", color:"#2A4060", cursor:"pointer", fontSize:12, transition:"color .2s" }}
          onMouseEnter={e=>e.target.style.color="#06b6d4"} onMouseLeave={e=>e.target.style.color="#2A4060"}>
          {navOpen?"◀":"▶"}
        </button>
      </div>
      ...
      <div style={{ flex:1, overflowY:"auto", position:"relative" }}>
        ...
        {/* Écran d'accueil si aucun module */}
        {!activeModule && (
